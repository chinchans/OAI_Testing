 ...
 */
 ...
 #include "F1AP_LTMCells-ToBeReleased-Item.h"
 #include "F1AP_LTMCFRAResourceConfig-Item.h"
 ...
 static f1ap_EarlySyncInformation_Request_t *cp_EarlySyncInformation_Request(const f1ap_EarlySyncInformation_Request_t *orig)
 {
   ...
   cp->RequestforRACHConfiguration = orig->RequestforRACHConfiguration;
 ...
 static bool eq_EarlySyncInformation_Request(const f1ap_EarlySyncInformation_Request_t *a, const f1ap_EarlySyncInformation_Request_t *b)
 {
   if (!a || !b)
     return a == b;
   if (a->RequestforRACHConfiguration != b->RequestforRACHConfiguration)
     return false;
 ...
 static f1ap_LTMInformation_Modify_t *cp_LTMInformation_Modify(const f1ap_LTMInformation_Modify_t *orig)
 {
   if (!orig)
     return NULL;
   f1ap_LTMInformation_Modify_t *cp = calloc_or_fail(1, sizeof(*cp));
   cp->LTMIndicator = orig->LTMIndicator;
   CP_OPT_BYTE_ARRAY(cp->ReferenceConfiguration, orig->ReferenceConfiguration);
   CP_OPT_BYTE_ARRAY(cp->cSIResourceConfigToAddModList, orig->cSIResourceConfigToAddModList);
   CP_OPT_BYTE_ARRAY(cp->cSIResourceConfigToReleaseList, orig->cSIResourceConfigToReleaseList);
 ...
 static void free_LTMInformation_Modify(f1ap_LTMInformation_Modify_t *p)
 {
   if (!p)
     return;
   FREE_OPT_BYTE_ARRAY(p->ReferenceConfiguration);
 ...
 static bool eq_LTMInformation_Modify(const f1ap_LTMInformation_Modify_t *a, const f1ap_LTMInformation_Modify_t *b)
 {
   if (!a || !b)
     return a == b;
   if (a->LTMIndicator != b->LTMIndicator)
 ...
 static f1ap_LTMCellsToBeReleasedList_t *cp_LTMCellsToBeReleasedList(const f1ap_LTMCellsToBeReleasedList_t *orig)
 {
   if (!orig || orig->list_count <= 0 || !orig->list_array)
     return NULL;
   f1ap_LTMCellsToBeReleasedList_t *cp = calloc_or_fail(1, sizeof(*cp));
   cp->list_count = orig->list_count;
   cp->list_array = calloc_or_fail((size_t)orig->list_count, sizeof(*cp->list_array));
   for (int i = 0; i < orig->list_count; ++i)
     cp->list_array[i] = orig->list_array[i];
 ...
 static void free_LTMCellsToBeReleasedList(f1ap_LTMCellsToBeReleasedList_t *p)
 {
   if (!p)
     return;
   free(p->list_array);
 ...
 static bool eq_LTMCellsToBeReleasedList(const f1ap_LTMCellsToBeReleasedList_t *a, const f1ap_LTMCellsToBeReleasedList_t *b)
 {
   if (!a || !b)
     return a == b;
   if (a->list_count != b->list_count)
     return false;
   for (int i = 0; i < a->list_count; ++i) {
     if (a->list_array[i].plmn.mcc != b->list_array[i].plmn.mcc || a->list_array[i].plmn.mnc != b->list_array[i].plmn.mnc
         || a->list_array[i].nr_cellid != b->list_array[i].nr_cellid)
       return false;
   }
   return true;
 }

 static f1ap_LTMCFRAResourceConfigList_t *cp_LTMCFRAResourceConfigList(const f1ap_LTMCFRAResourceConfigList_t *orig)
 {
   if (!orig || orig->list_count <= 0 || !orig->list_array)
     return NULL;
   f1ap_LTMCFRAResourceConfigList_t *cp = calloc_or_fail(1, sizeof(*cp));
   cp->list_count = orig->list_count;
   cp->list_array = calloc_or_fail((size_t)orig->list_count, sizeof(*cp->list_array));
   for (int i = 0; i < orig->list_count; ++i) {
     cp->list_array[i].cellID_plmn = orig->list_array[i].cellID_plmn;
     cp->list_array[i].cellID_nr_cellid = orig->list_array[i].cellID_nr_cellid;
     CP_OPT_BYTE_ARRAY(cp->list_array[i].lTMCFRAResourceConfig, orig->list_array[i].lTMCFRAResourceConfig);
     CP_OPT_BYTE_ARRAY(cp->list_array[i].lTMCFRAResourceConfigSUL, orig->list_array[i].lTMCFRAResourceConfigSUL);
   }
   return cp;
 }

 static void free_LTMCFRAResourceConfigList(f1ap_LTMCFRAResourceConfigList_t *p)
 {
   if (!p)
     return;
   for (int i = 0; i < p->list_count; ++i) {
     FREE_OPT_BYTE_ARRAY(p->list_array[i].lTMCFRAResourceConfig);
     FREE_OPT_BYTE_ARRAY(p->list_array[i].lTMCFRAResourceConfigSUL);
   }
   free(p->list_array);
   free(p);
 }

 static bool eq_LTMCFRAResourceConfigList(const f1ap_LTMCFRAResourceConfigList_t *a, const f1ap_LTMCFRAResourceConfigList_t *b)
 {
   if (!a || !b)
     return a == b;
   if (a->list_count != b->list_count)
     return false;
   for (int i = 0; i < a->list_count; ++i) {
     if (a->list_array[i].cellID_plmn.mcc != b->list_array[i].cellID_plmn.mcc
         || a->list_array[i].cellID_plmn.mnc != b->list_array[i].cellID_plmn.mnc
         || a->list_array[i].cellID_nr_cellid != b->list_array[i].cellID_nr_cellid)
       return false;
     _F1_EQ_CHECK_OPTIONAL_IE((&a->list_array[i]), (&b->list_array[i]), lTMCFRAResourceConfig, eq_ba);
     _F1_EQ_CHECK_OPTIONAL_IE((&a->list_array[i]), (&b->list_array[i]), lTMCFRAResourceConfigSUL, eq_ba);
   }
   return true;
 }

 static f1ap_LTMResetInformation_t *cp_LTMResetInformation(const f1ap_LTMResetInformation_t *orig)
 {
   if (!orig)
     return NULL;
   f1ap_LTMResetInformation_t *cp = calloc_or_fail(1, sizeof(*cp));
   CP_OPT_BYTE_ARRAY(cp->servingCellL2ResetConfiguration, orig->servingCellL2ResetConfiguration);
   return cp;
 }

 static void free_LTMResetInformation(f1ap_LTMResetInformation_t *p)
 {
   if (!p)
     return;
   FREE_OPT_BYTE_ARRAY(p->servingCellL2ResetConfiguration);
   free(p);
 }

 static bool eq_LTMResetInformation(const f1ap_LTMResetInformation_t *a, const f1ap_LTMResetInformation_t *b)
 {
   if (!a || !b)
     return a == b;
   _F1_EQ_CHECK_OPTIONAL_IE(a, b, servingCellL2ResetConfiguration, eq_ba);
   return true;
 }

 static f1ap_LTMTCIStatesConfigurationsList_t *cp_LTMTCIStatesConfigurationsList(const f1ap_LTMTCIStatesConfigurationsList_t *orig)
 {
   if (!orig || orig->list_count <= 0 || !orig->list_array)
     return NULL;
   f1ap_LTMTCIStatesConfigurationsList_t *cp = calloc_or_fail(1, sizeof(*cp));
   cp->list_count = orig->list_count;
   cp->list_array = calloc_or_fail((size_t)orig->list_count, sizeof(*cp->list_array));
   for (int i = 0; i < orig->list_count; ++i) {
     cp->list_array[i].tCIStateID = orig->list_array[i].tCIStateID;
     cp->list_array[i].tCIState = copy_byte_array(orig->list_array[i].tCIState);
   }
   return cp;
 }

 static void free_LTMTCIStatesConfigurationsList(f1ap_LTMTCIStatesConfigurationsList_t *p)
 {
   if (!p)
     return;
   for (int i = 0; i < p->list_count; ++i)
     free_byte_array(p->list_array[i].tCIState);
   free(p->list_array);
   free(p);
 }

 static bool eq_LTMTCIStatesConfigurationsList(const f1ap_LTMTCIStatesConfigurationsList_t *a,
                                               const f1ap_LTMTCIStatesConfigurationsList_t *b)
 {
   if (!a || !b)
     return a == b;
   if (a->list_count != b->list_count)
     return false;
   for (int i = 0; i < a->list_count; ++i) {
     if (a->list_array[i].tCIStateID != b->list_array[i].tCIStateID)
       return false;
     if (!eq_byte_array(&a->list_array[i].tCIState, &b->list_array[i].tCIState))
       return false;
   }
   return true;
 }

 static F1AP_LTMInformation_Modify_t encode_LTMInformation_Modify(const f1ap_LTMInformation_Modify_t *in)
 {
   F1AP_LTMInformation_Modify_t out = {0};
   out.lTMIndicator = F1AP_LTMIndicator_true;
   if (in->ReferenceConfiguration && in->ReferenceConfiguration->buf && in->ReferenceConfiguration->len > 0) {
     asn1cCalloc(out.referenceConfiguration, rc);
     rc->present = F1AP_ReferenceConfiguration_PR_referenceConfiguration;
     OCTET_STRING_fromBuf(&rc->choice.referenceConfiguration,
                          (const char *)in->ReferenceConfiguration->buf,
                          in->ReferenceConfiguration->len);
   }
   if ((in->cSIResourceConfigToAddModList && in->cSIResourceConfigToAddModList->buf)
       || (in->cSIResourceConfigToReleaseList && in->cSIResourceConfigToReleaseList->buf)) {
     asn1cCalloc(out.cSIResourceConfiguration, csi);
     if (in->cSIResourceConfigToAddModList && in->cSIResourceConfigToAddModList->buf) {
       asn1cCalloc(csi->cSIResourceConfigToAddModList, os);
       OCTET_STRING_fromBuf(os,
                            (const char *)in->cSIResourceConfigToAddModList->buf,
                            in->cSIResourceConfigToAddModList->len);
     }
     if (in->cSIResourceConfigToReleaseList && in->cSIResourceConfigToReleaseList->buf) {
       asn1cCalloc(csi->cSIResourceConfigToReleaseList, os);
       OCTET_STRING_fromBuf(os,
                            (const char *)in->cSIResourceConfigToReleaseList->buf,
                            in->cSIResourceConfigToReleaseList->len);
     }
   }
   return out;
 }

 static bool decode_LTMInformation_Modify(f1ap_LTMInformation_Modify_t **out, const F1AP_LTMInformation_Modify_t *in)
 {
   DevAssert(out != NULL && in != NULL);
   f1ap_LTMInformation_Modify_t *m = calloc_or_fail(1, sizeof(*m));
   m->LTMIndicator = (int)in->lTMIndicator;
   if (in->referenceConfiguration
       && in->referenceConfiguration->present == F1AP_ReferenceConfiguration_PR_referenceConfiguration) {
     const OCTET_STRING_t *os = &in->referenceConfiguration->choice.referenceConfiguration;
     m->ReferenceConfiguration = malloc_or_fail(sizeof(*m->ReferenceConfiguration));
     *m->ReferenceConfiguration = create_byte_array(os->size, os->buf);
   }
   if (in->cSIResourceConfiguration) {
     if (in->cSIResourceConfiguration->cSIResourceConfigToAddModList) {
       const OCTET_STRING_t *os = in->cSIResourceConfiguration->cSIResourceConfigToAddModList;
       m->cSIResourceConfigToAddModList = malloc_or_fail(sizeof(*m->cSIResourceConfigToAddModList));
       *m->cSIResourceConfigToAddModList = create_byte_array(os->size, os->buf);
     }
     if (in->cSIResourceConfiguration->cSIResourceConfigToReleaseList) {
       const OCTET_STRING_t *os = in->cSIResourceConfiguration->cSIResourceConfigToReleaseList;
       m->cSIResourceConfigToReleaseList = malloc_or_fail(sizeof(*m->cSIResourceConfigToReleaseList));
       *m->cSIResourceConfigToReleaseList = create_byte_array(os->size, os->buf);
     }
   }
   *out = m;
   return true;
 ...
 static F1AP_EarlySyncInformation_Request_t encode_EarlySyncInformation_Request(const f1ap_EarlySyncInformation_Request_t *in)
 {
   F1AP_EarlySyncInformation_Request_t out = {0};
   out.requestforRACHConfiguration = F1AP_RequestforRACHConfiguration_true;
   int n = in->LTMgNB_DU_IDsList_count;
   if (n < 1)
     n = 1;
   if (n > F1AP_MAX_NO_LTM_GNB_DU_IDS)
     n = F1AP_MAX_NO_LTM_GNB_DU_IDS;
   for (int i = 0; i < n; ++i) {
     asn1cSequenceAdd(out.lTMgNB_DU_IDsList.list, F1AP_LTMgNB_DU_IDs_Item_t, item);
     uint64_t du_id = (in->LTMgNB_DU_IDsList_array && i < in->LTMgNB_DU_IDsList_count)
                          ? in->LTMgNB_DU_IDsList_array[i].lTMgNB_DU_ID
                          : 0;
     if (du_id > 68719476735ULL)
       du_id = 68719476735ULL;
     asn_uint642INTEGER(&item->lTMgNB_DU_ID, du_id);
   }
   return out;
 }

 static bool decode_EarlySyncInformation_Request(f1ap_EarlySyncInformation_Request_t **out,
                                                 const F1AP_EarlySyncInformation_Request_t *in)
 {
   DevAssert(out != NULL && in != NULL);
   f1ap_EarlySyncInformation_Request_t *m = calloc_or_fail(1, sizeof(*m));
   m->RequestforRACHConfiguration = (int)in->requestforRACHConfiguration;
   m->LTMgNB_DU_IDsList_count = in->lTMgNB_DU_IDsList.list.count;
   if (m->LTMgNB_DU_IDsList_count > 0) {
     m->LTMgNB_DU_IDsList_array = calloc_or_fail((size_t)m->LTMgNB_DU_IDsList_count, sizeof(*m->LTMgNB_DU_IDsList_array));
     for (int i = 0; i < m->LTMgNB_DU_IDsList_count; ++i) {
       const F1AP_LTMgNB_DU_IDs_Item_t *item = (const F1AP_LTMgNB_DU_IDs_Item_t *)in->lTMgNB_DU_IDsList.list.array[i];
       unsigned long du_id = 0;
       asn_INTEGER2ulong(&item->lTMgNB_DU_ID, &du_id);
       m->LTMgNB_DU_IDsList_array[i].lTMgNB_DU_ID = du_id;
     }
   }
   *out = m;
   return true;
 ...
 static F1AP_LTMConfigurationIDMappingList_t encode_LTMConfigurationIDMappingList(const f1ap_LTMConfigurationIDMappingList_t *in)
 {
   F1AP_LTMConfigurationIDMappingList_t out = {0};
   int n = in->list_count;
   if (n > F1AP_MAX_NO_LTM_CONFIG_ID_MAPPING)
     n = F1AP_MAX_NO_LTM_CONFIG_ID_MAPPING;
   for (int i = 0; i < n; ++i) {
     asn1cSequenceAdd(out.list, F1AP_LTMConfigurationIDMapping_Item_t, item);
     const f1ap_LTMConfigurationIDMapping_Item_t *src = &in->list_array[i];
     const plmn_id_t *plmn = &src->lTMCellID_plmn;
     MCC_MNC_TO_PLMNID(plmn->mcc, plmn->mnc, plmn->mnc_digit_length, &item->lTMCellID.pLMN_Identity);
     NR_CELL_ID_TO_BIT_STRING(src->lTMCellID_nr_cellid, &item->lTMCellID.nRCellIdentity);
     uint8_t id = src->lTMConfigurationID;
     if (id < 1)
       id = 1;
     if (id > 8)
       id = 8;
     item->lTMConfigurationID = id;
   }
   return out;
 }

 static bool decode_LTMConfigurationIDMappingList(f1ap_LTMConfigurationIDMappingList_t **out,
                                                  const F1AP_LTMConfigurationIDMappingList_t *in)
 {
   DevAssert(out != NULL && in != NULL);
   int n = in->list.count;
   if (n <= 0) {
   ...
   f1ap_LTMConfigurationIDMappingList_t *m = calloc_or_fail(1, sizeof(*m));
   m->list_count = n;
   m->list_array = calloc_or_fail((size_t)n, sizeof(*m->list_array));
   for (int i = 0; i < n; ++i) {
     const F1AP_LTMConfigurationIDMapping_Item_t *item = (const F1AP_LTMConfigurationIDMapping_Item_t *)in->list.array[i];
     PLMNID_TO_MCC_MNC(&item->lTMCellID.pLMN_Identity,
                       m->list_array[i].lTMCellID_plmn.mcc,
                       m->list_array[i].lTMCellID_plmn.mnc,
                       m->list_array[i].lTMCellID_plmn.mnc_digit_length);
     BIT_STRING_TO_NR_CELL_IDENTITY(&item->lTMCellID.nRCellIdentity, m->list_array[i].lTMCellID_nr_cellid);
     m->list_array[i].lTMConfigurationID = (uint8_t)item->lTMConfigurationID;
   }
   *out = m;
   return true;
 }

 static F1AP_LTMCells_ToBeReleased_List_t encode_LTMCellsToBeReleasedList(const f1ap_LTMCellsToBeReleasedList_t *in)
 {
   F1AP_LTMCells_ToBeReleased_List_t out = {0};
   for (int i = 0; i < in->list_count; ++i) {
     asn1cSequenceAdd(out.list, F1AP_LTMCells_ToBeReleased_Item_t, item);
     const plmn_id_t *plmn = &in->list_array[i].plmn;
     MCC_MNC_TO_PLMNID(plmn->mcc, plmn->mnc, plmn->mnc_digit_length, &item->nRCGI.pLMN_Identity);
     NR_CELL_ID_TO_BIT_STRING(in->list_array[i].nr_cellid, &item->nRCGI.nRCellIdentity);
   }
   return out;
 }

 static bool decode_LTMCellsToBeReleasedList(f1ap_LTMCellsToBeReleasedList_t **out, const F1AP_LTMCells_ToBeReleased_List_t *in)
 {
   DevAssert(out != NULL && in != NULL);
   int n = in->list.count;
   if (n <= 0) {
   ...
   f1ap_LTMCellsToBeReleasedList_t *m = calloc_or_fail(1, sizeof(*m));
   m->list_count = n;
   m->list_array = calloc_or_fail((size_t)n, sizeof(*m->list_array));
   for (int i = 0; i < n; ++i) {
     const F1AP_LTMCells_ToBeReleased_Item_t *item = (const F1AP_LTMCells_ToBeReleased_Item_t *)in->list.array[i];
     PLMNID_TO_MCC_MNC(&item->nRCGI.pLMN_Identity,
                       m->list_array[i].plmn.mcc,
                       m->list_array[i].plmn.mnc,
                       m->list_array[i].plmn.mnc_digit_length);
     BIT_STRING_TO_NR_CELL_IDENTITY(&item->nRCGI.nRCellIdentity, m->list_array[i].nr_cellid);
   }
   *out = m;
   return true;
 }

 static F1AP_LTMCFRAResourceConfig_List_t encode_LTMCFRAResourceConfigList(const f1ap_LTMCFRAResourceConfigList_t *in)
 {
   F1AP_LTMCFRAResourceConfig_List_t out = {0};
   for (int i = 0; i < in->list_count; ++i) {
     asn1cSequenceAdd(out.list, F1AP_LTMCFRAResourceConfig_Item_t, item);
     const f1ap_LTMCFRAResourceConfig_Item_t *src = &in->list_array[i];
     const plmn_id_t *plmn = &src->cellID_plmn;
     MCC_MNC_TO_PLMNID(plmn->mcc, plmn->mnc, plmn->mnc_digit_length, &item->nRCGI.pLMN_Identity);
     NR_CELL_ID_TO_BIT_STRING(src->cellID_nr_cellid, &item->nRCGI.nRCellIdentity);
     if (src->lTMCFRAResourceConfig && src->lTMCFRAResourceConfig->buf) {
       asn1cCalloc(item->lTMCFRAResourceConfig, os);
       OCTET_STRING_fromBuf(os, (const char *)src->lTMCFRAResourceConfig->buf, src->lTMCFRAResourceConfig->len);
     }
     if (src->lTMCFRAResourceConfigSUL && src->lTMCFRAResourceConfigSUL->buf) {
       asn1cCalloc(item->lTMCFRAResourceConfigSUL, os);
       OCTET_STRING_fromBuf(os, (const char *)src->lTMCFRAResourceConfigSUL->buf, src->lTMCFRAResourceConfigSUL->len);
     }
   }
   return out;
 }

 static bool decode_LTMCFRAResourceConfigList(f1ap_LTMCFRAResourceConfigList_t **out, const F1AP_LTMCFRAResourceConfig_List_t *in)
 {
   DevAssert(out != NULL && in != NULL);
   int n = in->list.count;
   if (n <= 0) {
   ...
   f1ap_LTMCFRAResourceConfigList_t *m = calloc_or_fail(1, sizeof(*m));
   m->list_count = n;
   m->list_array = calloc_or_fail((size_t)n, sizeof(*m->list_array));
   for (int i = 0; i < n; ++i) {
     const F1AP_LTMCFRAResourceConfig_Item_t *item = (const F1AP_LTMCFRAResourceConfig_Item_t *)in->list.array[i];
     PLMNID_TO_MCC_MNC(&item->nRCGI.pLMN_Identity,
                       m->list_array[i].cellID_plmn.mcc,
                       m->list_array[i].cellID_plmn.mnc,
                       m->list_array[i].cellID_plmn.mnc_digit_length);
     BIT_STRING_TO_NR_CELL_IDENTITY(&item->nRCGI.nRCellIdentity, m->list_array[i].cellID_nr_cellid);
     if (item->lTMCFRAResourceConfig) {
       m->list_array[i].lTMCFRAResourceConfig = malloc_or_fail(sizeof(*m->list_array[i].lTMCFRAResourceConfig));
       *m->list_array[i].lTMCFRAResourceConfig =
           create_byte_array(item->lTMCFRAResourceConfig->size, item->lTMCFRAResourceConfig->buf);
     }
     if (item->lTMCFRAResourceConfigSUL) {
       m->list_array[i].lTMCFRAResourceConfigSUL = malloc_or_fail(sizeof(*m->list_array[i].lTMCFRAResourceConfigSUL));
       *m->list_array[i].lTMCFRAResourceConfigSUL =
           create_byte_array(item->lTMCFRAResourceConfigSUL->size, item->lTMCFRAResourceConfigSUL->buf);
     }
   }
   *out = m;
   return true;
 ...
 static F1AP_LTMResetInformation_t encode_LTMResetInformation(const f1ap_LTMResetInformation_t *in)
 {
   F1AP_LTMResetInformation_t out = {0};
   if (in->servingCellL2ResetConfiguration && in->servingCellL2ResetConfiguration->buf) {
     asn1cCalloc(out.servingCellL2ResetConfiguration, os);
     OCTET_STRING_fromBuf(os,
                          (const char *)in->servingCellL2ResetConfiguration->buf,
                          in->servingCellL2ResetConfiguration->len);
   }
   return out;
 }

 static bool decode_LTMResetInformation(f1ap_LTMResetInformation_t **out, const F1AP_LTMResetInformation_t *in)
 {
   DevAssert(out != NULL && in != NULL);
   f1ap_LTMResetInformation_t *m = calloc_or_fail(1, sizeof(*m));
   if (in->servingCellL2ResetConfiguration) {
     m->servingCellL2ResetConfiguration = malloc_or_fail(sizeof(*m->servingCellL2ResetConfiguration));
     *m->servingCellL2ResetConfiguration =
         create_byte_array(in->servingCellL2ResetConfiguration->size, in->servingCellL2ResetConfiguration->buf);
   }
   *out = m;
   return true;
 }

 static F1AP_LTMConfiguration_t encode_LTMConfiguration(const f1ap_LTMConfiguration_t *in)
 {
   F1AP_LTMConfiguration_t out = {0};
   int n = in->sSBInformation_count;
   if (n < 1)
     n = 1;
   for (int i = 0; i < n; ++i) {
     asn1cSequenceAdd(out.sSBInformation.sSBInformationList.list, F1AP_SSBInformationItem_t, item);
     const f1ap_SSBInformation_Item_t *src =
         (in->sSBInformation_array && i < in->sSBInformation_count) ? &in->sSBInformation_array[i] : NULL;
     F1AP_SSB_TF_Configuration_t *cfg = &item->sSB_Configuration;
     long freq = src ? src->sSB_frequency : 0;
     if (freq < 0)
       freq = 0;
     if (freq > 3279165)
       freq = 3279165;
     cfg->sSB_frequency = freq;
     long scs = src ? src->sSB_subcarrier_spacing : F1AP_SSB_TF_Configuration__sSB_subcarrier_spacing_kHz15;
     if (scs < 0)
       scs = 0;
     if (scs > 6)
       scs = 6;
     cfg->sSB_subcarrier_spacing = scs;
     long pwr = src ? src->sSB_Transmit_power : 0;
     if (pwr < -60)
       pwr = -60;
     if (pwr > 50)
       pwr = 50;
     cfg->sSB_Transmit_power = pwr;
     long per = src ? src->sSB_periodicity : F1AP_SSB_TF_Configuration__sSB_periodicity_ms20;
     if (per < 0)
       per = 0;
     if (per > 5)
       per = 5;
     cfg->sSB_periodicity = per;
     long hfo = src ? src->sSB_half_frame_offset : 0;
     cfg->sSB_half_frame_offset = (hfo < 0) ? 0 : (hfo > 1 ? 1 : hfo);
     long sfn = src ? src->sSB_SFN_offset : 0;
     cfg->sSB_SFN_offset = (sfn < 0) ? 0 : (sfn > 15 ? 15 : sfn);
     long pci = src ? src->pCI_NR : 0;
     if (pci < 0)
       pci = 0;
     if (pci > 1007)
       pci = 1007;
   ...
   if (in->referenceConfigurationInformation && in->referenceConfigurationInformation->buf) {
     asn1cCalloc(out.referenceConfigurationInformation, os);
     OCTET_STRING_fromBuf(os,
                          (const char *)in->referenceConfigurationInformation->buf,
                          in->referenceConfigurationInformation->len);
   }
   if (in->completeCandidateConfigurationIndicator) {
     asn1cCallocOne(out.completeCandidateConfigurationIndicator, F1AP_CompleteCandidateConfigurationIndicator_complete);
   }
   if (in->lTMCFRAResourceConfig && in->lTMCFRAResourceConfig->buf) {
     asn1cCalloc(out.lTMCFRAResourceConfig, os);
     OCTET_STRING_fromBuf(os, (const char *)in->lTMCFRAResourceConfig->buf, in->lTMCFRAResourceConfig->len);
   }
   if (in->lTMCFRAResourceConfigSUL && in->lTMCFRAResourceConfigSUL->buf) {
     asn1cCalloc(out.lTMCFRAResourceConfigSUL, os);
     OCTET_STRING_fromBuf(os, (const char *)in->lTMCFRAResourceConfigSUL->buf, in->lTMCFRAResourceConfigSUL->len);
   }
   return out;
 }

 static bool decode_LTMConfiguration(f1ap_LTMConfiguration_t **out, const F1AP_LTMConfiguration_t *in)
 {
   DevAssert(out != NULL && in != NULL);
   f1ap_LTMConfiguration_t *m = calloc_or_fail(1, sizeof(*m));
   m->sSBInformation_count = in->sSBInformation.sSBInformationList.list.count;
   if (m->sSBInformation_count > 0) {
     m->sSBInformation_array = calloc_or_fail((size_t)m->sSBInformation_count, sizeof(*m->sSBInformation_array));
     for (int i = 0; i < m->sSBInformation_count; ++i) {
       const F1AP_SSBInformationItem_t *item =
           (const F1AP_SSBInformationItem_t *)in->sSBInformation.sSBInformationList.list.array[i];
       const F1AP_SSB_TF_Configuration_t *cfg = &item->sSB_Configuration;
       m->sSBInformation_array[i].sSB_frequency = cfg->sSB_frequency;
       m->sSBInformation_array[i].sSB_subcarrier_spacing = cfg->sSB_subcarrier_spacing;
       m->sSBInformation_array[i].sSB_Transmit_power = cfg->sSB_Transmit_power;
       m->sSBInformation_array[i].sSB_periodicity = cfg->sSB_periodicity;
       m->sSBInformation_array[i].sSB_half_frame_offset = cfg->sSB_half_frame_offset;
       m->sSBInformation_array[i].sSB_SFN_offset = cfg->sSB_SFN_offset;
       m->sSBInformation_array[i].pCI_NR = item->pCI_NR;
     }
   }
   if (in->referenceConfigurationInformation) {
     m->referenceConfigurationInformation = malloc_or_fail(sizeof(*m->referenceConfigurationInformation));
     *m->referenceConfigurationInformation =
         create_byte_array(in->referenceConfigurationInformation->size, in->referenceConfigurationInformation->buf);
   }
   if (in->completeCandidateConfigurationIndicator) {
     m->completeCandidateConfigurationIndicator = calloc_or_fail(1, sizeof(*m->completeCandidateConfigurationIndicator));
     *m->completeCandidateConfigurationIndicator = (int)*in->completeCandidateConfigurationIndicator;
   }
   if (in->lTMCFRAResourceConfig) {
     m->lTMCFRAResourceConfig = malloc_or_fail(sizeof(*m->lTMCFRAResourceConfig));
     *m->lTMCFRAResourceConfig = create_byte_array(in->lTMCFRAResourceConfig->size, in->lTMCFRAResourceConfig->buf);
   }
   if (in->lTMCFRAResourceConfigSUL) {
     m->lTMCFRAResourceConfigSUL = malloc_or_fail(sizeof(*m->lTMCFRAResourceConfigSUL));
     *m->lTMCFRAResourceConfigSUL = create_byte_array(in->lTMCFRAResourceConfigSUL->size, in->lTMCFRAResourceConfigSUL->buf);
   }
   *out = m;
   return true;
 }
 ...
 static f1ap_LTMConfiguration_t *cp_LTMConfiguration(const f1ap_LTMConfiguration_t *orig)
 {
   ...
   cp->sSBInformation_count = orig->sSBInformation_count;
   if (orig->sSBInformation_count > 0 && orig->sSBInformation_array) {
     cp->sSBInformation_array = calloc_or_fail((size_t)orig->sSBInformation_count, sizeof(*cp->sSBInformation_array));
     for (int i = 0; i < orig->sSBInformation_count; ++i)
       cp->sSBInformation_array[i] = orig->sSBInformation_array[i];
   }
 ...
 static bool eq_LTMConfiguration(const f1ap_LTMConfiguration_t *a, const f1ap_LTMConfiguration_t *b)
 {
   if (!a || !b)
     return a == b;
   if (a->sSBInformation_count != b->sSBInformation_count)
     return false;
   for (int i = 0; i < a->sSBInformation_count; ++i) {
     const f1ap_SSBInformation_Item_t *x = &a->sSBInformation_array[i];
     const f1ap_SSBInformation_Item_t *y = &b->sSBInformation_array[i];
     if (x->sSB_frequency != y->sSB_frequency || x->sSB_subcarrier_spacing != y->sSB_subcarrier_spacing
         || x->sSB_Transmit_power != y->sSB_Transmit_power || x->sSB_periodicity != y->sSB_periodicity
         || x->sSB_half_frame_offset != y->sSB_half_frame_offset || x->sSB_SFN_offset != y->sSB_SFN_offset
         || x->pCI_NR != y->pCI_NR)
       return false;
   }
 ...
 F1AP_F1AP_PDU_t *encode_ue_context_mod_req(const f1ap_ue_context_mod_req_t *req)
 {
   ...
   /* Optional LTM IEs for Inter-gNB-DU LTM Handover (TS 38.473 9.2.2.7) */
   if (req->LTMInformation_Modify) {
     asn1cSequenceAdd(out->protocolIEs.list, F1AP_UEContextModificationRequestIEs_t, ie_ltm);
     ie_ltm->id = F1AP_ProtocolIE_ID_id_LTMInformation_Modify;
     ie_ltm->criticality = F1AP_Criticality_reject;
     ie_ltm->value.present = F1AP_UEContextModificationRequestIEs__value_PR_LTMInformation_Modify;
     ie_ltm->value.choice.LTMInformation_Modify = encode_LTMInformation_Modify(req->LTMInformation_Modify);
   }
   if (req->LTMCFRAResourceConfigList && req->LTMCFRAResourceConfigList->list_count > 0) {
     asn1cSequenceAdd(out->protocolIEs.list, F1AP_UEContextModificationRequestIEs_t, ie_cfra);
     ie_cfra->id = F1AP_ProtocolIE_ID_id_LTMCFRAResourceConfig_List;
     ie_cfra->criticality = F1AP_Criticality_ignore;
     ie_cfra->value.present = F1AP_UEContextModificationRequestIEs__value_PR_LTMCFRAResourceConfig_List;
     ie_cfra->value.choice.LTMCFRAResourceConfig_List = encode_LTMCFRAResourceConfigList(req->LTMCFRAResourceConfigList);
   }
   if (req->LTMConfigurationIDMappingList && req->LTMConfigurationIDMappingList->list_count > 0) {
     asn1cSequenceAdd(out->protocolIEs.list, F1AP_UEContextModificationRequestIEs_t, ie_map);
     ie_map->id = F1AP_ProtocolIE_ID_id_LTMConfigurationIDMappingList;
     ie_map->criticality = F1AP_Criticality_reject;
     ie_map->value.present = F1AP_UEContextModificationRequestIEs__value_PR_LTMConfigurationIDMappingList;
     ie_map->value.choice.LTMConfigurationIDMappingList =
         encode_LTMConfigurationIDMappingList(req->LTMConfigurationIDMappingList);
   }
   if (req->EarlySyncInformation_Request) {
     asn1cSequenceAdd(out->protocolIEs.list, F1AP_UEContextModificationRequestIEs_t, ie_es);
     ie_es->id = F1AP_ProtocolIE_ID_id_EarlySyncInformation_Request;
     ie_es->criticality = F1AP_Criticality_ignore;
     ie_es->value.present = F1AP_UEContextModificationRequestIEs__value_PR_EarlySyncInformation_Request;
     ie_es->value.choice.EarlySyncInformation_Request =
         encode_EarlySyncInformation_Request(req->EarlySyncInformation_Request);
   }
   if (req->LTMCellsToBeReleasedList && req->LTMCellsToBeReleasedList->list_count > 0) {
     asn1cSequenceAdd(out->protocolIEs.list, F1AP_UEContextModificationRequestIEs_t, ie_rel);
     ie_rel->id = F1AP_ProtocolIE_ID_id_LTMCells_ToBeReleased_List;
     ie_rel->criticality = F1AP_Criticality_reject;
     ie_rel->value.present = F1AP_UEContextModificationRequestIEs__value_PR_LTMCells_ToBeReleased_List;
     ie_rel->value.choice.LTMCells_ToBeReleased_List = encode_LTMCellsToBeReleasedList(req->LTMCellsToBeReleasedList);
   }
   if (req->LTMResetInformation) {
     asn1cSequenceAdd(out->protocolIEs.list, F1AP_UEContextModificationRequestIEs_t, ie_rst);
     ie_rst->id = F1AP_ProtocolIE_ID_id_LTMResetInformation;
     ie_rst->criticality = F1AP_Criticality_ignore;
     ie_rst->value.present = F1AP_UEContextModificationRequestIEs__value_PR_LTMResetInformation;
     ie_rst->value.choice.LTMResetInformation = encode_LTMResetInformation(req->LTMResetInformation);
   }
   /* TODO: LTMTCIStatesConfigurationsList has no UEContextModificationRequest ProtocolIE in R18.6.0 ASN.1 */
   ...
   for (int i = 0; i < in->protocolIEs.list.count; ++i) {
     ...
     switch (ie->id) {
       ...
       case F1AP_ProtocolIE_ID_id_LowerLayerPresenceStatusChange:
         ...
         break;
       case F1AP_ProtocolIE_ID_id_LTMInformation_Modify:
         _F1_EQ_CHECK_INT(ie->value.present, F1AP_UEContextModificationRequestIEs__value_PR_LTMInformation_Modify);
         _F1_CHECK_EXP(decode_LTMInformation_Modify(&out->LTMInformation_Modify, &ie->value.choice.LTMInformation_Modify));
         break;
       case F1AP_ProtocolIE_ID_id_LTMCFRAResourceConfig_List:
         _F1_EQ_CHECK_INT(ie->value.present, F1AP_UEContextModificationRequestIEs__value_PR_LTMCFRAResourceConfig_List);
         _F1_CHECK_EXP(decode_LTMCFRAResourceConfigList(&out->LTMCFRAResourceConfigList, &ie->value.choice.LTMCFRAResourceConfig_List));
         break;
       case F1AP_ProtocolIE_ID_id_LTMConfigurationIDMappingList:
         _F1_EQ_CHECK_INT(ie->value.present, F1AP_UEContextModificationRequestIEs__value_PR_LTMConfigurationIDMappingList);
         _F1_CHECK_EXP(
             decode_LTMConfigurationIDMappingList(&out->LTMConfigurationIDMappingList, &ie->value.choice.LTMConfigurationIDMappingList));
         break;
       case F1AP_ProtocolIE_ID_id_EarlySyncInformation_Request:
         _F1_EQ_CHECK_INT(ie->value.present, F1AP_UEContextModificationRequestIEs__value_PR_EarlySyncInformation_Request);
         _F1_CHECK_EXP(
             decode_EarlySyncInformation_Request(&out->EarlySyncInformation_Request, &ie->value.choice.EarlySyncInformation_Request));
         break;
       case F1AP_ProtocolIE_ID_id_LTMCells_ToBeReleased_List:
         _F1_EQ_CHECK_INT(ie->value.present, F1AP_UEContextModificationRequestIEs__value_PR_LTMCells_ToBeReleased_List);
         _F1_CHECK_EXP(decode_LTMCellsToBeReleasedList(&out->LTMCellsToBeReleasedList, &ie->value.choice.LTMCells_ToBeReleased_List));
         break;
       case F1AP_ProtocolIE_ID_id_LTMResetInformation:
         _F1_EQ_CHECK_INT(ie->value.present, F1AP_UEContextModificationRequestIEs__value_PR_LTMResetInformation);
         _F1_CHECK_EXP(decode_LTMResetInformation(&out->LTMResetInformation, &ie->value.choice.LTMResetInformation));
         break;
       default:
         PRINT_ERROR("F1AP_ProtocolIE_ID_id %ld unknown, skipping\n", ie->id);
         break;
 ...
 f1ap_ue_context_mod_req_t cp_ue_context_mod_req(const f1ap_ue_context_mod_req_t *orig)
 {
   ...
   if (orig->LTMInformation_Modify)
     cp.LTMInformation_Modify = cp_LTMInformation_Modify(orig->LTMInformation_Modify);
   if (orig->LTMConfigurationIDMappingList)
     cp.LTMConfigurationIDMappingList = cp_LTMConfigurationIDMappingList(orig->LTMConfigurationIDMappingList);
   if (orig->EarlySyncInformation_Request)
     cp.EarlySyncInformation_Request = cp_EarlySyncInformation_Request(orig->EarlySyncInformation_Request);
   if (orig->LTMCellsToBeReleasedList)
     cp.LTMCellsToBeReleasedList = cp_LTMCellsToBeReleasedList(orig->LTMCellsToBeReleasedList);
   if (orig->LTMCFRAResourceConfigList)
     cp.LTMCFRAResourceConfigList = cp_LTMCFRAResourceConfigList(orig->LTMCFRAResourceConfigList);
   if (orig->LTMResetInformation)
     cp.LTMResetInformation = cp_LTMResetInformation(orig->LTMResetInformation);
   if (orig->LTMTCIStatesConfigurationsList)
     cp.LTMTCIStatesConfigurationsList = cp_LTMTCIStatesConfigurationsList(orig->LTMTCIStatesConfigurationsList);
   return cp;
 }
 ...
 bool eq_ue_context_mod_req(const f1ap_ue_context_mod_req_t *a, const f1ap_ue_context_mod_req_t *b)
 {
   ...
   _F1_CHECK_EXP(eq_LTMInformation_Modify(a->LTMInformation_Modify, b->LTMInformation_Modify));
   _F1_CHECK_EXP(eq_LTMConfigurationIDMappingList(a->LTMConfigurationIDMappingList, b->LTMConfigurationIDMappingList));
   _F1_CHECK_EXP(eq_EarlySyncInformation_Request(a->EarlySyncInformation_Request, b->EarlySyncInformation_Request));
   _F1_CHECK_EXP(eq_LTMCellsToBeReleasedList(a->LTMCellsToBeReleasedList, b->LTMCellsToBeReleasedList));
   _F1_CHECK_EXP(eq_LTMCFRAResourceConfigList(a->LTMCFRAResourceConfigList, b->LTMCFRAResourceConfigList));
   _F1_CHECK_EXP(eq_LTMResetInformation(a->LTMResetInformation, b->LTMResetInformation));
   _F1_CHECK_EXP(eq_LTMTCIStatesConfigurationsList(a->LTMTCIStatesConfigurationsList, b->LTMTCIStatesConfigurationsList));
   return true;
 }
 ...
 void free_ue_context_mod_req(f1ap_ue_context_mod_req_t *req)
 {
   ...
   free(req->status);
   if (req->LTMInformation_Modify) {
     free_LTMInformation_Modify(req->LTMInformation_Modify);
     req->LTMInformation_Modify = NULL;
   }
   if (req->LTMConfigurationIDMappingList) {
     free_LTMConfigurationIDMappingList(req->LTMConfigurationIDMappingList);
     req->LTMConfigurationIDMappingList = NULL;
   ...
   if (req->LTMCellsToBeReleasedList) {
     free_LTMCellsToBeReleasedList(req->LTMCellsToBeReleasedList);
     req->LTMCellsToBeReleasedList = NULL;
   }
   if (req->LTMCFRAResourceConfigList) {
     free_LTMCFRAResourceConfigList(req->LTMCFRAResourceConfigList);
     req->LTMCFRAResourceConfigList = NULL;
   }
   if (req->LTMResetInformation) {
     free_LTMResetInformation(req->LTMResetInformation);
     req->LTMResetInformation = NULL;
   }
   if (req->LTMTCIStatesConfigurationsList) {
     free_LTMTCIStatesConfigurationsList(req->LTMTCIStatesConfigurationsList);
     req->LTMTCIStatesConfigurationsList = NULL;
   }
 }
   ...
   for (int i = 0; i < in->protocolIEs.list.count; ++i) {
     ...
     switch (ie->id) {
       ...
       case F1AP_ProtocolIE_ID_id_LTMConfiguration:
         _F1_EQ_CHECK_INT(ie->value.present, F1AP_UEContextModificationResponseIEs__value_PR_LTMConfiguration);
         _F1_CHECK_EXP(decode_LTMConfiguration(&out->LTMConfiguration, &ie->value.choice.LTMConfiguration));
         break;
       default:
         PRINT_ERROR("F1AP_ProtocolIE_ID_id %ld unknown, skipping\n", ie->id);
         break;
