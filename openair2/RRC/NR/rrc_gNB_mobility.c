                                                   ...
                                                   const nr_rrc_du_container_t *source_du)
{
  ...
  req->LTMInformation_Setup->LTMIndicator = 0; /* LTMIndicator ::= true */
  ...
  *req->EarlySyncInformation_Request->RequestforRACHConfiguration = create_byte_array(0, NULL);
  ...
  if (source_du != NULL)
    req->EarlySyncInformation_Request->LTMgNB_DU_IDsList_array[0].lTMgNB_DU_ID = source_du->setup_req->gNB_DU_id;
}
                                 ...
                                 bool ltm_handover)
{
  ...
  *ue_agg_mbr = 1000000000 /*bps*/;
  ...
  if (ltm_handover)
    fill_inter_du_ltm_ue_context_setup_req(&ue_context_setup_req, target_du, source_du);
