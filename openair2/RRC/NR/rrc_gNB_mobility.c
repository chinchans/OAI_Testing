...
void nr_rrc_fill_ue_context_mod_req_ltm_handover(f1ap_ue_context_mod_req_t *req, gNB_RRC_UE_t *ue)
{
  ...
  *req->nr_cellid = target_cell->nr_cellid;
  ...
  f1ap_LTMCFRAResourceConfigList_t *cfra_list = calloc_or_fail(1, sizeof(*cfra_list));
  cfra_list->list_count = 1;
  cfra_list->list_array = calloc_or_fail(1, sizeof(*cfra_list->list_array));
  cfra_list->list_array[0].cellID_plmn = target_cell->plmn;
  cfra_list->list_array[0].cellID_nr_cellid = target_cell->nr_cellid;
  req->LTMCFRAResourceConfigList = cfra_list;

  if (ue->ho_context->source && ue->ho_context->source->du) {
    const f1ap_served_cell_info_t *source_cell = &ue->ho_context->source->du->setup_req->cell[0].info;
    f1ap_LTMCellsToBeReleasedList_t *cells_rel = calloc_or_fail(1, sizeof(*cells_rel));
    cells_rel->list_count = 1;
    cells_rel->list_array = calloc_or_fail(1, sizeof(*cells_rel->list_array));
    cells_rel->list_array[0].plmn = source_cell->plmn;
    cells_rel->list_array[0].nr_cellid = source_cell->nr_cellid;
    req->LTMCellsToBeReleasedList = cells_rel;
  }
}
