...
void nr_HO_F1_trigger_telnet(gNB_RRC_INST *rrc, uint32_t rrc_ue_id)
{
  ...
  /* CI telnet `trigger_f1_ho` / `trigger_f1_ltm_ho` must use LTM path so
   * candidate-DU UE Context Setup carries LTMInformation_Setup /
   * LTMConfigurationIDMappingList / EarlySyncInformation_Request (Intent-1).
   * Plain F1 HO omits those IEs from the wire and PCAP. */
